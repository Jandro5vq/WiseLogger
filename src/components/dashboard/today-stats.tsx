...
interface TodayStatsProps {
  entryStartTime: string           // ISO — drives the fixed expectedEnd
  completedWorkedMinutes: number   // sum of finished tasks (server value)
...
export function TodayStats({
  ...
  // ── workday clock (independent of tasks) ──────────────────────────────────
  const expectedEndMs = new Date(entryStartTime).getTime() + (expectedMinutes + totalBreakMinutes) * 60_000
  ...
  return (
    <div className="grid grid-cols-2 gap-4 mb-6">
      ...
      <div className="rounded-lg border border-border bg-card p-4">
        <p className="text-xs text-muted-foreground uppercase tracking-wide">Worked today</p>
        ...
        <p className="text-xs text-muted-foreground mt-1">of {formatMinutes(expectedMinutes)}</p>
      </div>
      ...
      <div className={`rounded-lg border bg-card p-4 ${isOvertime ? 'border-green-500/50' : 'border-border'}`}>
        <p className="text-xs text-muted-foreground uppercase tracking-wide">End of day</p>
        ...
        {isOvertime ? (
          <p className="text-xs text-green-600 dark:text-green-400 mt-1 font-medium tabular-nums">
            +{formatMinutes(Math.abs(remainingMs) / 60_000)} overtime
          </p>
        ) : (
          <p className="text-xs text-muted-foreground mt-1 tabular-nums">
            {formatMinutes(remainingMs / 60_000)} remaining
          </p>
        )}
      </div>
    </div>
